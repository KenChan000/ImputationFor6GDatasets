...
from baselines.IGRM.models.gnn_model import get_gnn
from baselines.IGRM.models.prediction_model import MLPNet
from baselines.IGRM.utils.plot_utils import plot_curve, plot_sample
from baselines.IGRM.utils.utils import build_optimizer, get_known_mask, mask_edge
from baselines.IGRM.training.gae import *

import pdb

def out_of_sample_test_gnn_mdi(data_test, impute_model, model, log_path, device, return_filled_X=True):
    ...
    with torch.no_grad():
        ...
        #all_train_labels = data_test.train_labels.clone().detach().to(device)
        ...
        test_edge_attr = data_test.test_edge_attr.clone().detach().to(device)
        #test_labels = data_test.test_labels.clone().detach().to(device)
...
def train_gnn_mdi(data, args, log_path, device=torch.device('cpu'), return_filled_X=False):
    ...
    for epoch in range(args.epochs):
        ...
        #====================================
        if epoch % reconstrct_frequency == 0:   # reconstruct friend network
            Z = gae_net(obob_adj_norm, x_embd[:num_ob].clone())
            num_nodes = Z.shape[0]
            N = 5
            num_per_batch = num_nodes // N
            ...
            for i in range(N):
                if i != N-1:
                    Z_i = Z[i * num_per_batch: (i + 1) * num_per_batch]
                else:
                    Z_i = Z[i * num_per_batch:]

                adj_logits = Z_i @ Z.T
                
                adj_new, edge_probs = sample_adj(adj_logits)
                obob_edge_weight.append(edge_probs[adj_new.bool()])

                adj_new = torch.nonzero(adj_new).T
                adj_new[0] += i * num_per_batch

                obob_edge_index.append(adj_new)
            
            obob_edge_index = torch.cat(obob_edge_index, dim=1)
            obob_edge_weight = torch.cat(obob_edge_weight)
        elif (epoch-1) % reconstrct_frequency == 0:
            obob_edge_weight = obob_edge_weight.detach()
        #====================================    
    ...
    # If return_filled_X is True, return the filled X matrix
    if return_filled_X: 
