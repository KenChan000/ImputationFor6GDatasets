...
from baselines.Missdiff_SDE.diffusion_utils import EDMLoss
import pdb 
...
class Model(nn.Module):
    ...
    def forward(self, x):
        # split the input back into raw data and mask
        raw_length = x.shape[1] // 2 
        x, mask = x[:, :raw_length], x[:, raw_length:]
        loss = self.loss_fn(self.denoise_fn_D, x, mask)
