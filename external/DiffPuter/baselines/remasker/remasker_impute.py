...
import time, os, json
from baselines.remasker.utils import NativeScaler, MAEDataset, adjust_learning_rate, get_dataset
import baselines.remasker.model_mae as model_mae
from torch.utils.data import DataLoader, RandomSampler
import sys
import timm.optim.optim_factory as optim_factory
from baselines.remasker.utils import get_args_parser
...
class ReMasker:
    ...
    def fit(self, X_raw: pd.DataFrame):
        ...
        for epoch in range(self.max_epochs):
            ...
            pbar.set_description(f"Epoch {epoch+1}/{self.max_epochs}")
            iter = 0
            
            #use tqdm to show progress and estimated time
            for iter, (samples, masks) in enumerate(pbar):
                print('Epoch {}/{} Iter {}/{}'.format(epoch, self.max_epochs, iter, len(dataloader)))
                ...
                if not math.isfinite(loss_value):
                    ...
                    #sys.exit(1)
                    #Liancheng: if nan, return the model
                    return self
