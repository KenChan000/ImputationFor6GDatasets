"""
Generic DeepSense (DeepSense 6G) data loading + preprocessing.
"""
...
def load_deepsense_clean(data_dir,
                         csv_name: str,
                         beam_label_col: str | None,
    ...
    if beam_label_col and beam_label_col in df.columns:
        parsed_argmax = np.argmax(pwr_matrix, axis=1) + 1
        match = (parsed_argmax == df[beam_label_col].values).mean()
        if verbose:
            print(f"Argmax matches {beam_label_col}: {match * 100:.2f}%")
    ...
    if verbose:
        ...
        print(f"Parsed shape: {clean.shape}  |  missing cells: {n_missing}")

    if dropna:
        before = len(clean)
        clean = clean.dropna(how="any").reset_index(drop=True)
        dropped = before - len(clean)
        if verbose:
            print(f"Dropped {dropped} incomplete row(s) "
                  f"({before} -> {len(clean)}); experiments require fully observed input.")
    elif verbose and int(clean.isna().sum().sum()):
        print("  WARNING: dropna=False and NaNs remain — amputation will fail validate().")
