"""
Scenario 33 (DeepSense) data loading + preprocessing.
"""
...
DROP_COLUMNS = [
    "index", "unit1_rgb", "unit1_pwr_60ghz", "unit1_radar", "unit1_loc", "unit2_loc",
    "unit1_beam", "unit1_max_pwr", "time_stamp", "seq_index", "unit2_spd_over_grnd_kmph", "unit2_altitude",
    "unit2_geo_sep", "unit2_mode_fix_type", "unit2_interpolated_position"
]
...
def load_scenario33_clean(data_dir,
                         csv_name: str = "scenario33.csv",
                         cache_path=None,
                         force: bool = False,
    ...
    if "unit1_beam" in df.columns:
        ...
        match = (parsed_argmax == df["unit1_beam"].values).mean()
        if verbose:
            print(f"Argmax matches unit1_beam: {match * 100:.2f}%")
